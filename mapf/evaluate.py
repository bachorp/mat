...
if __name__ == '__main__':
    ...
    for c, d in cs.items():
        ...
        for i, s in ps.items():
            ...
            data[i][c] = \
                (s['t_total'].mean(), s['t_total'].std(), s['t_extend'].mean(), s['t_extend'].std())

    print(" g   b   c |    Without preprocessing*   |     With preprocessing*")
    for k, v in data.items():
        p   = v['1|2|600|4|0|0']
        nop = v['0|2|600|4|0|0']
        print("{:2}  {:2}  {:2} | {:5.2f} [{:4.2f}] - {:5.2f} [{:4.2f}] | {:5.2f} [{:4.2f}] - {:5.2f} [{:4.2f}]".format(k[0], k[1], k[3], nop[0] * 1e-3, nop[1] * 1e-3, nop[2] * 1e-3, nop[3] * 1e-3, p[0] * 1e-3, p[1] * 1e-3, p[2] * 1e-3, p[3] * 1e-3))
    ...
    for k, v in data.items():
        p   = v['1|2|600|4|0|0']
        nop = v['0|2|600|4|0|0']
        print("{:2}  {:2}  {:2} | {:5.2f} | {:5.2f} ".format(k[0], k[1], k[3], nop[0] * 1e-3 - nop[2] * 1e-3, p[0] * 1e-3 - p[2] * 1e-3))
    print("*Solver time")
