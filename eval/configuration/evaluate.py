...
def load(encoding=False) -> dict:
    ...
    for c, d in cs.items():
        ...
        for i, s in ps.items():
            ...
            t = s.apply(
                lambda x: pd.NA if x["result"] == "Timeout" else max(x["t_total"], 1),
                axis=1,
                result_type="reduce",
            )
...
if __name__ == "__main__":
    ...
    out = (
        "{} vs. {}: Sum of difference of logarithms is {:.2f}, {} vs. {} solved ({:+})."
    )
    ...
    print(
        out.format(
            "With edge variables (2)", "without edge variables (1)", i, s1, s2, s1 - s2
        )
    )
    next(cyc)
    ...
    print(
        out.format(
            "With movement variables (3)",
            "without movement variables (2)",
            i,
            s1,
            s2,
            s1 - s2,
