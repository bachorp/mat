...
class Animation:
  def __init__(self, map, schedule):
    ...
    # create agents:
    self.T = 0
    # draw container tasks first
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      tcolors = iter(plt.cm.gist_rainbow(np.linspace(0, 1, 10))) # TODO the '10' should probably be set dynamically (num tasks)
      for task in d["potentialTasks"]:
        start = task[0]
        goal = task[1]
        tcolor = next(tcolors)
        self.patches.append(Rectangle((start[0] - 0.25, start[1] - 0.25), 0.5, 0.5, facecolor=tcolor, edgecolor='black', alpha=0.5, linestyle='--'))
        self.patches.append(Rectangle((goal[0] - 0.25, goal[1] - 0.25), 0.5, 0.5, facecolor=tcolor, edgecolor='black', alpha=0.5))
