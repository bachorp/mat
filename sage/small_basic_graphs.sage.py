# This script verifies that all basic graphs with less than 7 nodes
# except T_0' induce a permutation group G ≥ A_n.
    ...
    for a in range(3, n):
        for b in range(1, n):
            ...
            for c in range(1, a):
                if not b + (a - c) + 2 > max(
                    a, b + c
                ):  # The outer cycle must be longer than α and β
                    continue
                α = S(tuple(i for i in range(1, a + 1)))
                β = S(
                    tuple(i for i in range(1, c + 1))
                    + tuple(i for i in range(a + 1, a + b + 1))
                )
                G = S.subgroup([α, β])
                if not (
                    G.is_isomorphic(AlternatingGroup(n))
                    or G.is_isomorphic(SymmetricGroup(n))
                ):
